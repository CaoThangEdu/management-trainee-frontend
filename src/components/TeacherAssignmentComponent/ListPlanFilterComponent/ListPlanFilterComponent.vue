        ...
        <div class="card-body">
              ...
              <div class="form-row filter-wrapper ml-0 mr-0">
                ...
                <div class="col-xl-2 col-md-4 col-sm-12">
                  <button
                    ...
                    @click="getInternshipCourseAsync"
                    title="Tìm kiếm"
                  >
          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item, index) in internshipCourse" :key="index">
                  <th scope="row">{{ index + 1 }}</th>
                  <td>{{ item.internshipCourseName }}</td>
                  <td>{{ convertTime(item.startDay, "DD/MM/YYYY") }}</td>
                  <td>{{ convertTime(item.endDay, "DD/MM/YYYY") }}</td>
                  <td>{{ getPlanStatusName(item.status) }}</td>
                  <td>
                    <router-link
                      class="btn btn-info"
                      :to="{
                        name: 'phan-cong-dot',
                        params: { internshipCourseId: item.id },
                      }"
                      title="Chi tiết"
                    >
                ...
                <tr
                  v-show="
                    internshipCourse == null || internshipCourse.length === 0
                  "
                >
